...
class HomePage(AbstractEmailForm):
    ...
    content_panels = AbstractEmailForm.content_panels + [
        ...
        MultiFieldPanel([
            InlinePanel('studies', label="Studies", min_num=1),
        ], heading="Education"),

        MultiFieldPanel([
            InlinePanel('jobs', label="Jobs", min_num=1),
        ], heading="Experience"),        
...
class Education(Orderable, ClusterableModel):
    """Model stores data about a project created by the user."""

    page = ParentalKey('home.HomePage', related_name="studies")
    degree = models.CharField(max_length=96)
    university = models.CharField(max_length=96)
    description_u = RichTextField(blank=True, null=True)
    start_date = models.DateTimeField("Start date")
    finished_date_e = models.DateTimeField("Finished date")

    panels = [
        FieldPanel('degree'),
        FieldPanel('university'),
        FieldPanel('description_u', classname="full"),
        FieldPanel('start_date'),
        FieldPanel('finished_date_e'),
    ]

    def clean(self):
        if self.finished_date_e > timezone.now():
            raise forms.ValidationError("Finished date cannot be in the future!")

class Experience(Orderable, ClusterableModel):
    """Model stores data about a project created by the user."""

    page = ParentalKey('home.HomePage', related_name="jobs")
    position = models.CharField(max_length=96)
    provider = models.CharField(max_length=96)
    description_j = RichTextField(blank=True, null=True)
    start_date_d = models.DateTimeField("Start date")
    finished_date_d = models.DateTimeField("Finished date")

    panels = [
        FieldPanel('position'),
        FieldPanel('provider'),
        FieldPanel('description_j', classname="full"),
        FieldPanel('start_date_d'),
        FieldPanel('finished_date_d'),
    ]

    def clean(self):
        if self.finished_date_d > timezone.now():
            raise forms.ValidationError("Finished date cannot be in the future!")
